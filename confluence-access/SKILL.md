...
只要任务需要“搜索 Confluence 页面并读取内容”，每一轮搜索都必须使用 views-ranked 两阶段流程。脚本已经把 `search-pages`、`search-cql` 和 JSON action `search` 改成默认执行这个流程；不要用原始搜索结果直接挑页面读取，除非 metadata 无法解析 views、用户明确要求不用 views，或任务只是调试/列出原始候选。
...
只有调试、确认 CQL、metadata 无法解析 views 时回退，或用户明确要求原始候选列表时，才使用 `--raw`、`search-pages-raw`、`search-cql-raw` 或 JSON action `search_raw`。
...
- Data Center 不使用 analytics REST API 或页面 HTML 查询 views。脚本通过 Confluence REST API 读取 content metadata / metadata properties，只解析元数据，不下载页面正文。
...
低层 raw 搜索命令（只用于调试、确认 CQL、metadata 无法解析 views 时的回退，或用户明确要求查看原始候选列表）：
...
在 Data Center 中，`get-page-views` 不调用 analytics API，也不抓页面 HTML；它只请求 `content/{pageId}?expand=metadata` 和必要时的 `content/{pageId}/property`，从元数据中解析浏览量。批量查询会并发执行，但不会下载正文。若无法解析，会返回 `viewsAvailable: false` 和失败原因；不要把不可用当作 views 为 0。
...
**强制默认规则**：只要任务需要“搜索 Confluence 页面并读取内容”，每一轮 Search 都必须执行 views-ranked 两阶段流程。`search-pages`、`search-cql` 和 JSON action `search` 已经默认执行该流程并读取 Top 5 页面正文。不要用 `--raw`、`search-pages-raw`、`search-cql-raw` 或 `search_raw` 的前几条结果进入 Read，除非 metadata 无法解析 views、用户明确要求不用 views，或任务只是调试/列出原始搜索结果。若跳过 views，必须在回答或工作记录中说明原因。
...
- **Search 阶段二（views 排序）**：优先从阶段一搜索结果的 expanded metadata 中解析 views；缺失时批量读取 `content/{pageId}?expand=metadata` 和必要时的 content properties。按 views 从高到低选择最多 5 条结果。只对这 5 条获取完整文档内容，进入后续分析。若 metadata 无法解析 views，说明限制，并退化为基于相关度、更新时间和标题/空间线索选择最多 5 条读取。
